...
- When the task is complete, fold its durable results into the project documents: `pisafe-design.md` for requirements and invariants, `IMPLEMENTATION_PROGRESS.md` for current behavior and verification, and `DECISIONS.md` for lasting decisions. Then delete the temporary task plan; living documents remain.
...
- Append entries to a **Decisions** section in the plan document as they occur, and repeat them in the increment's commit message or summary so they surface without opening the file. Fold decisions that remain relevant after the task into `DECISIONS.md` when the task lands.
...
`IMPLEMENTATION_PROGRESS.md` and `DECISIONS.md` describe the project as it exists now; they are not append-only history. Git and commit messages preserve superseded states.

- A change is incomplete until every affected claim in both documents has been updated or removed. Search by concept; do not assume adding one new entry is sufficient.
- `IMPLEMENTATION_PROGRESS.md` contains only current behavior, verification, known gaps, and next work. Delete completed gaps and obsolete mechanisms.
- `DECISIONS.md` contains only decisions that still constrain the project. When a decision is reversed, replace its old entry with the current decision and rationale. Retain rejected alternatives only when they still explain the present design.
- Never resolve disagreement by blindly trusting either documentation or code. Investigate, then make the code, tests, design, progress, and decisions agree in the same increment.
- Before completing an increment, ask: _what documentation did this change make false, and did I remove or rewrite it?_
...
- **Docs are testimony, not ground truth.** `pisafe-design.md` is the authority on what must hold, `IMPLEMENTATION_PROGRESS.md` records what currently happens and has been verified, and `DECISIONS.md` records why. Detailed claims may still be stale or wrong; when documentation and code disagree, investigate rather than preserving complexity because a comment claims it is needed.
