...
// Has reports whether a login is stored. It asks nothing about what is stored,
// so a credential that can no longer be parsed or refreshed is still something
// Forget can be asked to take away.
func (store Keychain) Has(ctx context.Context) (bool, error) {
	_, err := store.secrets.Load(ctx, Name)
