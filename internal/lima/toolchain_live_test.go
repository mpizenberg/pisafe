...
func TestLiveTheToolchainIsReachableAndNeverShadowed(t *testing.T) {
	...
	projectKey := liveProject(t, transport, "livetools")
	spec := liveRun(t, ctx, transport, projectKey, imageID, "livetools-"+stamp)
	...
	invoked := inContainer(t, ctx, transport, spec, `
...
`)
	...
	shadowed := inContainer(t, ctx, transport, spec, `
printf '#!/bin/sh\necho shadowed\n' >"$HOME/.local/bin/git"
